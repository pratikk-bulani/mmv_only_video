...
def process_samples(features_dict, num_frames=32, stride=1, is_training=True,
  ...
  # print(video.shape) # (None, None, None, 3)
  ...
  # print(video.shape, min_resize) # (320, None, None, 3) 224
  video = resize_smallest(video, min_resize)
  # print(video.shape) # (320, None, None, 3)
  ...
  # print(video.shape) # (320, 224, 224, 3)
...
def reshape_windows(features_dict, num_frames):
  ...
  # print(x.shape, num_frames) # (None, 320, 224, 224, 3) 32
  x = tf.reshape(x, (-1, num_frames, x.shape[2], x.shape[3], x.shape[4]))
  # print(x.shape) # (None, 32, 224, 224, 3). I thought (10, 32, 224, 224, 3)
...
def main(argv):
  ...
  for test_ex in test_examples:
    test_ex['video'] = np.zeros(shape = (2, 200, 640, 360, 3)) # dummy input for checking purpose
    # print(test_ex['video'].shape, type(test_ex['video'])) # (10, 32, 224, 224, 3) <class 'numpy.ndarray'>
    vid_representation_test, _ = forward_apply(params=params,
                                               state=state,
                                               images=test_ex['video'],
                                               audio_spectrogram=dummy_audio,
                                               word_ids=dummy_word_ids)
    # print(type(vid_representation_test), type(_), vid_representation_test.shape) # <class 'jaxlib.xla_extension.DeviceArray'> <class 'dict'> (10, 4096)
