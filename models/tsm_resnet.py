...
class TSMResNetV2(hk.Module):
  ...
  def __call__(
    ...
    net_1 = hk.MaxPool(
        ...
        padding='SAME')(
            net_1)
    if self._final_endpoint == end_point: return net_1
    results.append(net_1)
    ...
    for unit_id, (channels, num_blocks, stride) in enumerate(
      ...
      net_2 = TSMResNetUnit(
          ...
          tsm_mode=tsm_mode,
          name=end_point)(
              results[-1], is_training=is_training)
      if self._final_endpoint == end_point:
        return net_2
      results.append(net_2)

    if self._normalize_fn is not None:
      net_3 = self._normalize_fn(results[-1], is_training=is_training)
    net_3 = jax.nn.relu(net_3)

    end_point = 'last_conv'
    results.append(net_3)
    if self._final_endpoint == end_point: return net_3
    return results
    # print(net.shape) # (#frames, 12, 20, 4096)
    net = jnp.mean(net, axis=(1, 2)) # (#frames, 4096)
    # Prepare embedding outputs for TSM (temporal average of features).
    # print(num_frames, tsm_mode) # #frames, gpu
