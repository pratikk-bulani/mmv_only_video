...
class AudioTextVideoEmbedding(hk.Module):
  ...
  def __call__(self,
    ...
    visual_representation = video_cnn(images, is_training=is_training, final_endpoint=final_endpoint)
    return {
        "vid_embd": None,
        "aud_embd": None,
        "txt_embd": None,
...
class VisualModule(hk.Module):
  ...
  def __call__(self, images, is_training, final_endpoint='Embeddings'):
    """Connects graph to images."""
    features = self._cnn(images, is_training=is_training, final_endpoint=final_endpoint)
    return features
